...
- `src/app/` routes. `/` is the landing page (static, no client JS; its
  screenshots live in `public/shots/`, regenerated by `npm run shots`),
  `/app` the v2 desktop, `/legacy` the prototype.
- `src/desktop/` the desktop: canvas, Window shape, window tool, window kinds
  registry (heavy kinds are `React.lazy`), cascading placement, the welcome
  tour, the Start here card, the keyboard map, problems (toast) and reset.
...
- `src/lib/` shared helpers (env, bundled tldraw assets, motion preference).
- `scripts/` build-static, the API doc generator and `shots.mjs` (landing
  screenshots from the production build).
...
- `e2e/` Playwright tests, including `a11y.spec.ts` (axe-core; serious and
  critical findings in our markup fail). `docs/` plan. `tasks/todo.md`
  checklist.
